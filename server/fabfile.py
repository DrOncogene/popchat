...
def ssl_setup(ctx: Context):
    ...
    ctx.run(
        "sudo certbot -n --agree-tos --nginx -m mypythtesting@gmail.com -d\
             pop-chat-api.droncogene.com",
        warn=True,
    )
...
def run_containers(ctx: Context):
    ...
    ctx.put("./.env", "/tmp/popchat/.env")
    ctx.run(f"docker compose -f /tmp/popchat/docker-compose.yml up -d")
    ctx.run(f"rm /tmp/popchat/.env")
