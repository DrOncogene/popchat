...
<li animate:flip={{duration: 500}}  on:click={ (e) => openChat(e) } on:keypress={ (e) => openChat(e) } data-type="{chat.type}" data-id="{chat.id}" class="relative flex shrink-0 items-center space-x-2 flex-nowrap h-[60px] py-2 px-6 mb-0.5 cursor-pointer hover:bg-sec-700 overflow-hidden transition-all duration-250 {selected}">
  <ProfileImage />
  ...
  {#if chat.msgCount > 0}
    <span class="absolute top-1/3 right-2 rounded-full bg-msg-blue shadow-md w-6 h-6 grid place-content-center text-sm font-semibold">{chat.msgCount}</span>
  {/if}
</li>
{:else}
